...
namespace ASC.Api.Documents
{
    ...
    public class PrivacyRoomController : ControllerBase
    {
        ...
        /// <visible>false</visible>
        [Read("access/{fileId}")]
        public async Task<IEnumerable<EncryptionKeyPair>> GetPublicKeysWithAccess(string fileId)
        {
            if (!PrivacyRoomSettings.GetEnabled(SettingsManager)) throw new System.Security.SecurityException();

            return await EncryptionKeyPairHelper.GetKeyPair(fileId, FileStorageService);
        }

        [Read("access/{fileId:int}")]
        public async Task<IEnumerable<EncryptionKeyPair>> GetPublicKeysWithAccess(int fileId)
        {
            if (!PrivacyRoomSettings.GetEnabled(SettingsManager)) throw new System.Security.SecurityException();

            return await EncryptionKeyPairHelper.GetKeyPair(fileId, FileStorageServiceInt);
        }
