...
namespace ASC.Web.Files
{
    ...
    public class FileHandlerService
    {
        ...
        private async Task CreateFile<T>(HttpContext context, T folderId)
        {
            ...
            context.Response.Redirect(
                (context.Request.Query["openfolder"].FirstOrDefault() ?? "").Equals("true")
                    ? await PathProvider.GetFolderUrlById(file.FolderID)
        ...
        private async Task Redirect(HttpContext context)
        {
            ...
            {
                await Redirect(context, q.FirstOrDefault() ?? "", q1.FirstOrDefault() ?? "");
            }
        }
        ...
        private async Task Redirect<T>(HttpContext context, T folderId, T fileId)
        {
            ...
            if (folderId != null)
            {
                try
                {
                    urlRedirect = await PathProvider.GetFolderUrlById(folderId);
                }
