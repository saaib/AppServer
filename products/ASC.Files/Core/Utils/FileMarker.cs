        ...
        internal async Task ExecMarkFileAsNew<T>(AsyncTaskData<T> obj)
        {
            ...
            else
            {
                ...
                if (obj.FileEntry.RootFolderType == FolderType.USER)
                {
                    ...
                    var folderShare = await folderDaoInt.GetFolder(await GlobalFolder.GetFolderShare(DaoFactory));

                    foreach (var userID in userIDs)
                    {
                        var userFolderId = await GlobalFolder.GetFolderMy(this, DaoFactory);
                        ...
                        if (obj.FileEntry.ProviderEntry)
                        {
                            rootFolder = obj.FileEntry.RootFolderCreator == userID
                                             ? await folderDaoInt.GetFolder(userFolderId)
                                             : folderShare;
                        }
                ...
                else if (obj.FileEntry.RootFolderType == FolderType.Privacy)
                {
                    foreach (var userID in userIDs)
                    {
                        var privacyFolderId = await folderDao.GetFolderIDPrivacy(false, userID);
                        if (Equals(privacyFolderId, 0)) continue;

                        var rootFolder = await folderDao.GetFolder(privacyFolderId);
                        if (rootFolder == null) continue;
