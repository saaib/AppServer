...
namespace ASC.Files.Core.Security
{
    ...
    public class FileSecurity : IFileSecurity
    {
        ...
        public Task<List<Tuple<FileEntry<T>, bool>>> CanRead<T>(IEnumerable<FileEntry<T>> entry)
        {
        ...
        public Task<bool> CanCustomFilterEdit<T>(FileEntry<T> entry, Guid userId)
        {
        ...
        public Task<bool> CanCustomFilterEdit<T>(FileEntry<T> entry)
        {
        ...
        private async Task<IEnumerable<FileEntry<T>>> Filter<T>(IEnumerable<FileEntry<T>> entries, FilesSecurityActions action, Guid userId)
        {
            ...
            if (entries.Any(filter))
            {
                ...
                foreach (var e in filteredEntries)
                {
                    ...
                    else if (action == FilesSecurityActions.CustomFilter && await adapter.CanCustomFilterEdit(e, userId))
                    {
        ...
        public async Task<List<FileEntry>> GetSharesForMe(FilterType filterType, bool subjectGroup, Guid subjectID, string searchText = "", bool searchInContent = false, bool withSubfolders = false)
        {
            ...
            result.AddRange(await GetSharesForMe<int>(records.Where(r=> r.EntryId.GetType() == typeof(int)), subjects, filterType, subjectGroup, subjectID, searchText, searchInContent, withSubfolders));
            result.AddRange(await GetSharesForMe<string>(records.Where(r => r.EntryId.GetType() == typeof(string)), subjects, filterType, subjectGroup, subjectID, searchText, searchInContent, withSubfolders));
            return result;
        }

        private async Task<List<FileEntry>> GetSharesForMe<T>(IEnumerable<FileShareRecord> records, List<Guid> subjects, FilterType filterType, bool subjectGroup, Guid subjectID, string searchText = "", bool searchInContent = false, bool withSubfolders = false)
        {
            var folderDao = daoFactory.GetFolderDao<T>();
            ...
            {
                var folders = await folderDao.GetFolders(folderIds.Keys, filterType, subjectGroup, subjectID, searchText, withSubfolders, false);
            ...
            {
                var filesInSharedFolders = await fileDao.GetFiles(folderIds.Keys, filterType, subjectGroup, subjectID, searchText, searchInContent);
        ...
        public async Task<List<FileEntry>> GetPrivacyForMe(FilterType filterType, bool subjectGroup, Guid subjectID, string searchText = "", bool searchInContent = false, bool withSubfolders = false)
        {
            var securityDao = daoFactory.GetSecurityDao<int>();
            var subjects = GetUserSubjects(AuthContext.CurrentAccount.ID);
            var records = securityDao.GetShares(subjects);
            ...
            var result = new List<FileEntry>();
            result.AddRange(await GetPrivacyForMe<int>(records.Where(r => r.EntryId.GetType() == typeof(int)), subjects, filterType, subjectGroup, subjectID, searchText, searchInContent, withSubfolders));
            result.AddRange(await GetPrivacyForMe<string>(records.Where(r => r.EntryId.GetType() == typeof(string)), subjects, filterType, subjectGroup, subjectID, searchText, searchInContent, withSubfolders));
            return result;
        }

        private async Task<List<FileEntry<T>>> GetPrivacyForMe<T>(IEnumerable<FileShareRecord> records, List<Guid> subjects, FilterType filterType, bool subjectGroup, Guid subjectID, string searchText = "", bool searchInContent = false, bool withSubfolders = false)
        {
            var folderDao = daoFactory.GetFolderDao<T>();
            var fileDao = daoFactory.GetFileDao<T>();
