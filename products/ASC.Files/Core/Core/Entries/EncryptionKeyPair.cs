...
namespace ASC.Web.Files.Core.Entries
{
    ...
    public class EncryptionKeyPairHelper
    {
        ...
        public async Task<IEnumerable<EncryptionKeyPair>> GetKeyPair<T>(T fileId, FileStorageService<T> FileStorageService)
        {
            ...
            if (!await FileSecurity.CanEdit(file)) throw new System.Security.SecurityException(FilesCommonResource.ErrorMassage_SecurityException_EditFile);
            if (file.RootFolderType != FolderType.Privacy) throw new NotSupportedException();

            var fileShares = (await FileStorageService.GetSharedInfo(new List<T> { fileId }, new List<T> { })).ToList();
